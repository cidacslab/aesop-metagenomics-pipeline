...
def write_kraken_report_summary(report_files, folder_path, output_file):
  # write header
  with open(output_file, "w") as file:
    file.write("file,kraken_unclass,kraken_class,kraken_viruses,kraken_bacteria")
    file.write(",kraken_archaea,kraken_eukaryota,kraken_human\n")
  ...
  for report_file in report_files:
    ...
    k2_viruses = KrakenParser.get_abundance(report_by_taxid, "10239")
    k2_bacteria = KrakenParser.get_abundance(report_by_taxid, "2")
    k2_archaea = KrakenParser.get_abundance(report_by_taxid, "2157")
    k2_eukaryota = KrakenParser.get_abundance(report_by_taxid, "2759")
    k2_human = KrakenParser.get_abundance(report_by_taxid, "9606")
    output_content += f"{report_file},{k2_unclass},{k2_class},{k2_viruses},"
    output_content += f"{k2_bacteria},{k2_archaea},{k2_eukaryota},{k2_human}\n"
...
def write_complete_summary(input_path, summary_name, fastq_folders):
  ...
  for folder in fastq_folders:
    ...
    if folder == "0-raw_samples" or folder == "1.3-fastp_output":
...
def main():
  ...
  # input_path = "/home/pedro/aesop/pipeline/results/viral_discovery_v1/dataset_mock"
