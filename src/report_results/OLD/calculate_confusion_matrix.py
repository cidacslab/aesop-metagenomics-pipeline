...
import taxonomy_tree_parser as TaxonomyParser
    ...
    for row in csv_reader:
      ...
      for i in range(1, 7):
        ...
        if taxid not in all_classified_tree:
          level = TaxonomyParser.Level(i + 3).name
          node = TaxonomyParser.TreeNode(names[i], taxid, level)
...
def calculate_confusion_matrix(accession_species_taxid, sample_total_reads, ground_truth_tree, true_positive_tree, all_classified_tree, output_file):
  ...
  for accession in accession_species_taxid:
    ...
    genus_node = accession_species_node.get_parent_by_level(TaxonomyParser.Level.G)
    if genus_node is not None:
      accession_genus_node = genus_node
    ...
    if accession_genus_node.level_enum != TaxonomyParser.Level.G:
      # genus_name = f"Undefined genus in {accession_genus_node.level.lower()} {genus_name}"
      genus_name = f"Undefined genus in {genus_name}"
...
def main():
  ...
  input_metadata_path = "./data/taxdump_20241211"  
