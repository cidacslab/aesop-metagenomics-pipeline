...
def main():
  ...
  pid = os.getpid()
  input_suffix = sys.argv[5]
  input_dir = sys.argv[6]
  output_dir = sys.argv[7]
  # nthreads=$8 
  align_identity = float(sys.argv[9])
  align_length = float(sys.argv[10])
  align_evalue = float(sys.argv[11])
  taxonomy_database = sys.argv[12]
  base_path = sys.argv[13]
  metadata_path = sys.argv[14]
  count_reads_folder = sys.argv[15]
  count_reads_extension = sys.argv[16]
  mapping_folder = sys.argv[17] if len(sys.argv) > 17 else ""
  kraken_folder = sys.argv[18] if len(sys.argv) > 18 else ""
  ...
  for input_file in get_files_in_folder(input_dir, input_suffix):
    # Print start message
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S+0000")
    print(f"\n\nB_PID: {pid} [{timestamp}]: Started task Input: {input_file}")
...
if __name__ == '__main__':
  ...
  f = open(f"{log_file}", "w", buffering=1)  # buffering=1 => line buffering
  sys.stdout = f
